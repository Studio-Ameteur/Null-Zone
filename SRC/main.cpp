...
static const int CREDITS_LINE_COUNT = sizeof(credits_lines)/sizeof(credits_lines[0]);
...
static void game_start_level(int level) {
    ...
    if(g_cheats.infiniteAmmo) {
        for(int i=0;i<MAX_WEAPONS;i++)
            g_player.weapons[i].ammo=g_player.weapons[i].maxAmmo;
    }
...
static void apply_cheat_skip_level() {
    if(!g_cheats_unlocked) return;
    game_next_level();
}
...
static void render_credits(SDL_Renderer* r, double dt) {
    ...
    g_credits_scroll-=dt*60.0;

    int lineH=sh/20;
    int totalH=CREDITS_LINE_COUNT*lineH;
    ...
    for(int i=0;i<CREDITS_LINE_COUNT;i++){
        ...
        else if(i>1&&credits_lines[i-1]&&strcmp(credits_lines[i-1],"================================")==0) c=HUD_ACCENT;
        ...
        if(surf){
            ...
            if(tex){
                SDL_Rect dst={sw/2-tw/2,y,surf->w,surf->h};
    ...
    if(g_credits_scroll+(double)(totalH)<0){
        if(g_credits_music){Mix_HaltMusic();Mix_FreeMusic(g_credits_music);g_credits_music=nullptr;}
    ...
    SDL_SetRenderDrawColor(r,HUD_GRAY.r,HUD_GRAY.g,HUD_GRAY.b,100);
    hud_text_center(r,g_font_small,"Press ESC to skip",sw/2,sh-30,HUD_GRAY);
}
...
static void render_cheat_menu(SDL_Renderer* r) {
    ...
    SDL_SetRenderDrawColor(r,5,3,10,220);
    ...
    struct CheatItem { const char* name; bool* val; };
    CheatItem items[]={
        {"GOD MODE",        &g_cheats.godMode},
        {"ALL WEAPONS",     &g_cheats.allWeapons},
        {"INFINITE AMMO",   &g_cheats.infiniteAmmo},
        {"SKIP LEVEL",      nullptr},
        {"UNLOCK ALL LEVELS",&g_cheats.allLevels},
    };
    int count=5;
    int lh=bh/7;
    ...
    for(int i=0;i<count;i++){
        ...
        Color nc=on?HUD_GREEN:HUD_WHITE;
        if(!items[i].val) nc=HUD_ORANGE;
        char buf[64];
        if(items[i].val)
            SDL_snprintf(buf,sizeof(buf),"%s: %s",items[i].name,on?"ON":"OFF");
        else
            SDL_snprintf(buf,sizeof(buf),"%s",items[i].name);
        hud_text_center(r,g_font_medium,buf,sw/2,iy,nc);
    }
    hud_text_center(r,g_font_small,"1-5: Toggle   ESC: Back",sw/2,by+bh-30,HUD_GRAY);
}
...
int WINAPI WinMain(HINSTANCE,HINSTANCE,LPSTR,int) {
    ...
    while(g_running&&!g_input.quit){
        ...
        if(showCheats){
            if(action_pause()) showCheats=false;
            for(int i=0;i<5;i++){
                if(key_pressed((SDL_Scancode)(SDL_SCANCODE_1+i))){
                    switch(i){
                    case 0: g_cheats.godMode=!g_cheats.godMode; break;
                    case 1: g_cheats.allWeapons=!g_cheats.allWeapons; break;
                    case 2: g_cheats.infiniteAmmo=!g_cheats.infiniteAmmo; break;
                    case 3: if(g_state==GS_PLAYING) apply_cheat_skip_level(); break;
                    case 4: g_cheats.allLevels=!g_cheats.allLevels; break;
                    }
                }
            }
            SDL_SetRenderDrawColor(g_renderer,0,0,0,255);
        ...
        case GS_PAUSED:{
            ...
            switch(act){
            case MACT_NEW_GAME:   game_new(); mouse_capture(true); break;
            case MACT_CONTINUE:
            case MACT_LOAD_OK:
            ...
            case MACT_RESUME:  g_state=GS_PLAYING; mouse_capture(true); break;
            case MACT_GOTO_MAIN: g_state=GS_MENU; mouse_capture(false); break;
            case MACT_QUIT:    g_running=false; break;
            default: break;
            }
            if(g_cheats_unlocked&&key_pressed(SDL_SCANCODE_C)&&g_state==GS_MENU)
                showCheats=true;
            break;
        }
        case GS_PLAYING:{
            ...
            if(g_cheats_unlocked&&key_pressed(SDL_SCANCODE_C)) showCheats=true;
            accumulator+=delta;
        ...
        if(g_state==GS_MENU||g_state==GS_PAUSED){
            ...
            if(g_cheats_unlocked){
                ...
                hud_text_center(g_renderer,g_font_small,
                    "[ C ] CHEATS",sw/2,sh-50,HUD_ORANGE);
            }
        }
