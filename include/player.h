...
static const char* weapon_names[MAX_WEAPONS] = {
    ...
    "RAILGUN","GRENADE LAUNCHER","MINIGUN","CRYO CANNON","CHAINSAW","BFG-9000"
};
...
inline void player_update_dir(Player& p) {
    ...
    p.plane={-sin(p.angle)*tan(FOV*0.5),cos(p.angle)*tan(FOV*0.5)};
}

inline bool player_try_move(Player& p, double dx, double dy) {
    const double R=0.25;
    double nx=p.pos.x+dx, ny=p.pos.y+dy;
...
inline void player_switch_weapon(Player& p, int dir) {
    ...
    for(int i=1;i<MAX_WEAPONS;i++){
        ...
        if(p.weapons[next].owned){p.currentWeapon=next;break;}
    }
}

inline void player_take_damage(Player& p, int dmg) {
    if(p.armor>0){int a=std::min(p.armor,dmg/2);p.armor-=a;dmg-=a;}
    p.health-=dmg;
    p.damageFlash=0.35;
...
inline void player_update(Player& p, double dt, bool& footstep) {
    ...
    double rotSpeed=g_mouse_sensitivity*(double)mouse_dx();
    ...
    p.footstepTimer+=dt;
    footstep=false;
    if(moving&&p.footstepTimer>=p.footstepInterval){
        p.footstepTimer=0.0;
        footstep=true;
    }
    if(!moving) p.footstepTimer=0.0;

    if(p.hackerEffect){p.hackerTimer-=dt;if(p.hackerTimer<=0)p.hackerEffect=false;}
...
inline bool player_can_fire(Player& p) {
    ...
    bool melee=(p.currentWeapon==(int)WPN_FISTS||p.currentWeapon==(int)WPN_CHAINSAW);
    if(melee) return fire();
...
inline PlayerStats player_get_stats(const Player& p) {
    ...
    s.armor=p.armor;   s.maxArmor=p.maxArmor;
    s.score=p.score;   s.kills=p.kills;
    s.level=p.level;   s.pos=p.pos;
    s.angle=p.angle;   s.currentWeapon=p.currentWeapon;
